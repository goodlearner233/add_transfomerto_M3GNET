...
"""
Full-dataset training script for the M3GNet Linear Atomic-Sum baseline.

Overview
--------
This script trains an M3GNet baseline on the complete MatPES-PBE-2025.2
dataset. It retains the same message-passing, three-body interaction, linear
atomic-energy head, data split, and training settings as the corresponding
M3GNet--Transformer experiment, but omits the Transformer encoder. The resulting
comparison isolates the effect of Transformer-based atomic-feature exchange.
...
The atomic-sum readout preserves the extensive form of the total energy.
...
Force and stress training consequently propagates derivatives through the
linear atomic-energy head and the M3GNet graph blocks.
...
Single-GPU Example
------------------
    python train_m3gnet_linear_atomic_sum_full_matpes.py \
        --data /path/to/MatPES-PBE-2025.2.json \
        --output-dir ./runs/full_matpes_m3gnet_linear \
        --max-epochs 200 \
        --batch-size 32 \
...
---------------
    python train_m3gnet_linear_atomic_sum_full_matpes.py \
        --data /path/to/MatPES-PBE-2025.2.json \
        --output-dir ./runs/full_matpes_m3gnet_linear \
        --resume ./runs/full_matpes_m3gnet_linear/checkpoints/last.ckpt
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the M3GNet linear atomic-sum baseline on the full MatPES dataset."
    )
    parser.add_argument("--data", type=Path, required=True, help="Path to the complete MatPES JSON file.")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("runs/m3gnet_linear_atomic_sum_full_matpes"),
...
def main() -> None:
    ...
    config["model_variant"] = "m3gnet_linear_atomic_sum_baseline"
    config["comparison_target"] = "m3gnet_transformer_linear_atomic_sum"
